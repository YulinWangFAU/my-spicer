...
class SensitivityModel(nn.Module):
    ...
    def forward(self, masked_kspace: torch.Tensor, ACS_center, ACS_size) -> torch.Tensor:

        x = torch.zeros_like(masked_kspace)

        x[:, :, :, ACS_center - ACS_size // 2:ACS_center + ACS_size // 2+ 1] = masked_kspace[:, :, :, ACS_center - ACS_size // 2:ACS_center + ACS_size // 2 + 1]
...
class SPNet(nn.Module):
    ...
    def __init__(
        ...
        self.cascades = nn.ModuleList(
            [SPNetBlock(NormUnet(chans, pools)) for _ in range(num_cascades)]
        )

    def forward(self, masked_kspace: torch.Tensor, mask: torch.Tensor, ACS_center, ACS_size) -> torch.Tensor:
        sens_maps = self.sens_net(masked_kspace, ACS_center, ACS_size)
        kspace_pred = masked_kspace.clone()

        for cascade in self.cascades:
            kspace_pred = cascade(kspace_pred, masked_kspace, mask, sens_maps)
        total_image = transpose_operator(kspace_pred, sens_maps, mask)
        return total_image, sens_maps
